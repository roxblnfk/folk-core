...
//!
//! The PHP-facing functions (folk_worker_ready/recv/send) are defined in
//! lib.rs (required by wrap_function! macro). This module provides the
...
/// A request sent from tokio to the worker.
pub struct TaskRequest {
    pub method: String,
    pub payload: Bytes,
    pub reply: oneshot::Sender<anyhow::Result<Bytes>>,
}
...
struct WorkerState {
    ...
    current_reply: Option<oneshot::Sender<anyhow::Result<Bytes>>>,
}
...
pub fn do_recv() -> Result<Option<(String, Vec<u8>)>, &'static str> {
    WORKER.with(|w| {
        ...
        match state.task_rx.blocking_recv() {
            Some(req) => {
...
pub fn do_send_error(message: String) -> Result<(), &'static str> {
    WORKER.with(|w| {
        let mut state = w.borrow_mut();
        let state = state.as_mut().ok_or("not in a worker thread")?;
